...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.meinestadt.de/deutschland/suche', {
      params: { words: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.search-result, .result, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('meinestadt', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/meinestadt]', err.message); return []; }
}
