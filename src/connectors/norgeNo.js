...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.norge.no/nb/sok', {
      params: { q: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.search-result, .result, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('norge-no', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/norge-no]', err.message); return []; }
}
