...
async function search(query) {
  try {
    const response = await httpClient.get('https://www.searchalot.com/', {
      params: { q: query },
    ...
    const results = [];
    $('.result, .web-result, .organic-result').each((i, el) => {
      ...
      if (title || url) results.push(normalise('searchalot', query, { title, url, snippet, rank: i + 1 }));
    });
    return results;
  } catch (err) { console.error('[connectors/searchalot]', err.message); return []; }
}
export { search };
