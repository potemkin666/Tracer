...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.openarchieven.nl/search.php', {
      params: { name: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.result, .search-result, tr, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('openarchieven', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/openarchieven]', err.message); return []; }
}
