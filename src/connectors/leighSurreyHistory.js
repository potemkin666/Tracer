...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://leigh-surrey.org.uk/directory/history/', {
      params: { q: query },
    ...
    $('.search-result, article, .result, li').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) url = 'https://leigh-surrey.org.uk' + url;
      const snippet = $(el).find('p, .description, .details').first().text().trim();
      if (title || url) results.push(normalise('leigh-surrey-history', query, { title, url, snippet, rank: i + 1 }));
    });
    return results;
  } catch (err) { console.error('[connectors/leigh-surrey-history]', err.message); return []; }
}
