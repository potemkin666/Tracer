...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.nibusinessinfo.co.uk/search', {
      params: { keys: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.search-result, .result, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('nibusinessinfo', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/nibusinessinfo]', err.message); return []; }
}
