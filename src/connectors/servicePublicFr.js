...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.service-public.fr/P10125/recherche', {
      params: { q: query },
    ...
    $('.result, .search-result, article').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) { url = 'https://www.service-public.fr' + url; }
      const snippet = $(el).find('p, .description').first().text().trim();
      if (title || url) { results.push(normalise('service-public-fr', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/service-public-fr]', err.message); return []; }
}
