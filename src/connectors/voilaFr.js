...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://search.ke.voila.fr/', {
      params: { q: query },
    ...
    $('.result, .search-result, article').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) { url = 'https://www.voila.fr' + url; }
      const snippet = $(el).find('p, .snippet').first().text().trim();
      if (title || url) { results.push(normalise('voila-fr', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/voila-fr]', err.message); return []; }
}
