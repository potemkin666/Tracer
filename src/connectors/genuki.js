...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.genuki.org.uk/search', {
      params: { q: query },
    ...
    $('.search-result, article, .result, li').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) url = 'https://www.genuki.org.uk' + url;
      const snippet = $(el).find('p, .description, .details').first().text().trim();
      if (title || url) results.push(normalise('genuki', query, { title, url, snippet, rank: i + 1 }));
    });
    return results;
  } catch (err) { console.error('[connectors/genuki]', err.message); return []; }
}
