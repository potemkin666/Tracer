...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.fntt.lt/', {
      params: { q: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.search-result, article, .result, li').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) url = 'https://www.fntt.lt' + url;
      const snippet = $(el).find('p, .description, .details').first().text().trim();
      if (title || url) results.push(normalise('fntt-lt', query, { title, url, snippet, rank: i + 1 }));
    });
    return results;
  } catch (err) { console.error('[connectors/fntt-lt]', err.message); return []; }
}
