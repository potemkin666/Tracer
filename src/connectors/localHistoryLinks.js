...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.local-history.co.uk/links/historical.html', {
      params: { q: query },
    ...
    $('.search-result, article, .result, li').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) url = 'https://www.local-history.co.uk' + url;
      const snippet = $(el).find('p, .description, .details').first().text().trim();
      if (title || url) results.push(normalise('local-history-links', query, { title, url, snippet, rank: i + 1 }));
    });
    return results;
  } catch (err) { console.error('[connectors/local-history-links]', err.message); return []; }
}
