...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.proff.dk/branches%C3%B8g', {
      params: { q: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.search-result, .company-listing, article, .result').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) url = 'https://www.proff.dk' + url;
      const snippet = $(el).find('p, .description, .details').first().text().trim();
      if (title || url) results.push(normalise('proff-dk', query, { title, url, snippet, rank: i + 1 }));
    });
    return results;
  } catch (err) { console.error('[connectors/proff-dk]', err.message); return []; }
}
