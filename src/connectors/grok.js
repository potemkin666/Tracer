...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://grok.com/', {
      params: { q: query },
    ...
    $('.result, .search-result, article').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) { url = 'https://grok.com' + url; }
      const snippet = $(el).find('p, .snippet').first().text().trim();
      if (title || url) { results.push(normalise('grok', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/grok]', err.message); return []; }
}
