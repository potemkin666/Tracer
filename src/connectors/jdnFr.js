...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.journaldunet.com/solutions/recherche/', {
      params: { q: query },
    ...
    $('.result, .search-result, article').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) { url = 'https://www.journaldunet.com' + url; }
      const snippet = $(el).find('p, .description, .snippet').first().text().trim();
      if (title || url) { results.push(normalise('jdn-fr', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/jdn-fr]', err.message); return []; }
}
