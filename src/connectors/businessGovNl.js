...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://business.gov.nl/search/', {
      params: { q: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.search-result, .result, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('business-gov-nl', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/business-gov-nl]', err.message); return []; }
}
