...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.118712.fr/', {
      params: { q: query },
    ...
    $('.result, .search-result, article').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) { url = 'https://www.118712.fr' + url; }
      const snippet = $(el).find('p, .address, .description').first().text().trim();
      if (title || url) { results.push(normalise('118712-fr', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/118712-fr]', err.message); return []; }
}
