...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://yellowpages.com.eg/en/search', {
      params: { q: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.result-item, .search-result, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('yellowpages-eg', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/yellowpages-eg]', err.message); return []; }
}
