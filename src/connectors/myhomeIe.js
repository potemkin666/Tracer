...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.myhome.ie/residential/search', {
      params: { q: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.property-card, .search-result, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('myhome-ie', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/myhome-ie]', err.message); return []; }
}
