...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.wiewaswie.nl/personen-zoeken/', {
      params: { q: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.result, .search-result, tr, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('wiewaswie', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/wiewaswie]', err.message); return []; }
}
