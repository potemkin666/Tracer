...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://egypt-business.com/search', {
      params: { q: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.listing, .search-result, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('egypt-business', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/egypt-business]', err.message); return []; }
}
