...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.cemeteryscribes.com/', {
      params: { q: query },
    ...
    $('.search-result, article, .result, li').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) url = 'https://www.cemeteryscribes.com' + url;
      const snippet = $(el).find('p, .description, .details').first().text().trim();
      if (title || url) results.push(normalise('cemetery-scribes', query, { title, url, snippet, rank: i + 1 }));
    });
    return results;
  } catch (err) { console.error('[connectors/cemetery-scribes]', err.message); return []; }
}
