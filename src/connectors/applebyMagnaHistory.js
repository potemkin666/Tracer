...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('http://www.applebymagna.org.uk/appleby_history/', {
      params: { q: query },
    ...
    $('.search-result, article, .result, li').each((i, el) => {
      ...
      if (url && !url.startsWith('http')) url = 'http://www.applebymagna.org.uk' + url;
      const snippet = $(el).find('p, .description, .details').first().text().trim();
      if (title || url) results.push(normalise('appleby-magna-history', query, { title, url, snippet, rank: i + 1 }));
    });
    return results;
  } catch (err) { console.error('[connectors/appleby-magna-history]', err.message); return []; }
}
