...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.dalil140.com/search', {
      params: { q: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.listing, .search-result, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('dalil140', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/dalil140]', err.message); return []; }
}
