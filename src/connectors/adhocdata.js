...
async function search(query, apiKeys = {}) {
  try {
    const response = await httpClient.get('https://www.adhocdata.nl/en/search', {
      params: { q: query },
      headers: {
        'User-Agent': UA,
        Accept: 'text/html',
    ...
    $('.result, .search-result, article').each((i, el) => {
      ...
      if (title || url) { results.push(normalise('adhocdata', query, { title, url, snippet, rank: i + 1 })); }
    });
    return results;
  } catch (err) { console.error('[connectors/adhocdata]', err.message); return []; }
}
